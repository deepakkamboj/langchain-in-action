...
<Callout type="info">
  **LangChain Documentation**: "LangChain is the platform for agent engineering.
  Build agents with any model provider and deploy them anywhere."
</Callout>
...
<Callout type="default">
  **Definition - LangChain**: A platform for agent engineering that provides
  frameworks, tools, and services to build, deploy, and observe AI agents that
  can reason, use tools, and interact with external systems.
</Callout>
...
<Callout type="warning">
  **Important**: LangChain agents are built on top of LangGraph, giving you
  access to all these advanced features even when using the simpler LangChain
  interface.
</Callout>
...
<Callout type="default">
  **Best Practice**: Start with LangChain for rapid prototyping, then use
  LangGraph when you need fine-grained control over agent behavior and execution
  flow.
</Callout>
...
<Callout type="info">
  **Tip**: This dramatic simplification is possible because LangChain handles
  all the infrastructure complexity behind a clean, composable interface.
</Callout>
...
<Callout type="default">
  **Best Practice**: Extend LangChain with custom components rather than working
  around the framework. This ensures you benefit from all platform features like
...
<Callout type="info">
  **Tip**: LangChain provides integrations to hundreds of LLMs and thousands of
  other integrations through independent provider packages, giving you maximum
...
<Callout type="warning">
  **Important**: LangChain uses independent provider packages. Only install the
  integrations you need to keep your dependencies clean and avoid conflicts.
</Callout>
...
<Callout type="error">
  **Security Consideration**: Never commit API keys to version control. Add
  `.env` to your `.gitignore` file immediately after creation.
</Callout>
...
<Callout type="info">
  **Tip**: For this example, you'll need to set up an Anthropic account and set
  the `ANTHROPIC_API_KEY` environment variable. The official docs recommend
  Claude Sonnet 4.5 for its excellent reasoning capabilities.
</Callout>
...
<Callout type="warning">
  **Important**: In production, use a persistent checkpointer that saves to a
  database. The in-memory version is only suitable for development and testing.
</Callout>
